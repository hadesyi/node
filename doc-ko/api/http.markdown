...
요청, 응답, 서버에 `'timeout'` 리스너를 추가하지 않으면 타임아웃되었을 때 소켓을 제거한다.
...
전체 데이터가 성공적으로 커널버퍼에 플러시되면 `true`를 반환한다. 전체 혹은 일부의 데이터가
...
`response.httpVersionMajor`는 첫번째 정수이고
`response.httpVersionMinor`는 두번째 정수이다.
...
사용하거나 `require('url').parse`에 두번째 인자로 `true`를 전달할 수 있다.
예제:
